...
class AccountAssetLine(models.Model):
    ...
    def _compute_values(self):
        ...
        for asset in asset_ids:
            grouped_dlines.append(
                dlines.filtered(lambda line: line.asset_id.id == asset.id)
            )
    ...
    def write(self, vals):
        for depreciation_line in self:
            line_date = vals.get("line_date") or depreciation_line.line_date
            asset_lines = depreciation_line.asset_id.depreciation_line_ids
            if list(vals.keys()) == ["move_id"] and not vals["move_id"]:
                ...
                depreciation_line.move_id
                and not self.env.context.get("allow_asset_line_update")
                and depreciation_line.type != "create"
            ):
                raise UserError(
                    _(
                ...
                if depreciation_line.type == "create":
                    check = asset_lines.filtered(
                    ...
                    check = asset_lines.filtered(
                        lambda al: al != depreciation_line
    ...
    def unlink(self):
        for depreciation_line in self:
            if depreciation_line.type == "create" and depreciation_line.amount:
                raise UserError(
                    _("You cannot remove an asset line " "of type 'Depreciation Base'.")
                )
            elif depreciation_line.move_id:
                raise UserError(
                    _(
            ...
            previous = depreciation_line.previous_id
            next_line = depreciation_line.asset_id.depreciation_line_ids.filtered(
                lambda line: line.previous_id == depreciation_line and line not in self
            )
            if next_line:
