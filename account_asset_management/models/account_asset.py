...
class AccountAsset(models.Model):
    ...
    def _check_recursion(self, parent=None):
        ...
        if not res:
            raise UserError(_("Error ! You can not create recursive assets."))
        return res
    ...
    def write(self, vals):
        ...
        for asset in self:
            ...
            if asset_type == "view" or self.env.context.get(
                "asset_validate_from_write"
            ):
                continue
            asset._create_first_asset_line()
    ...
    def open_entries(self):
        ...
        am_ids = [line.move_id.id for line in amls]
        return {
        ...
        for asset in self:
            ...
            if posted_lines:
                ...
                depreciated_value_posted = depreciated_value = sum(
                    [line.amount for line in posted_lines]
                )
    ...
    def _get_fy_duration_factor(self, entry, firstyear):
        ...
        if self.prorata:
            if firstyear:
                depreciation_date_start = fields.Datetime.from_string(self.date_start)
                ...
                if fy_id:
                    ...
                    first_fy_year_factor = self._get_fy_duration(fy_id, option="years")
                    duration_factor = (
    ...
    def _get_depreciation_stop_date(self, depreciation_start_date):
        if self.method_time == "year":
            ...
            depreciation_stop_date = fields.Datetime.from_string(self.method_end)
    ...
    def _compute_depreciation_table(self):
        ...
        if fy:
            ...
            if asset_date_start > fy_date_start:
                asset_ref = (
                    self.code and "%s (ref: %s)" % (self.name, self.code) or self.name
                )
                raise UserError(
                    _(
    ...
    def _compute_entries(self, date_end, check_triggers=False):
        ...
        for depreciation in depreciations:
            try:
                ...
                asset_ref = (
                    asset.code
                    and "%s (ref: %s)" % (asset.name, asset.code)
                    or asset.name
                )
