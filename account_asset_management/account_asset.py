...
class account_asset_depreciation_line(orm.Model):
    ...
    def _get_previous_id(self, cr, uid, context=None):
        """
        We block the creation of new lines via the User Interface
        via the _defaults function since OpenERP 6.1 has
        no standard way to disable UI create in a one2many.
        Allowing this requires adding logic to recalculate
        a number of depreciation table fields.
        The recommended way at this point in time is that the end user
        generates the table via the 'Compute' button and adjusts
        dates and amounts in case it doesn't match with his needs.
        """
        if not context:
            context = {}
        if 'create' in context and not context['create']:
            raise orm.except_orm(
                _('Error!'),
                _("You cannot add a depreciation line, "
                  "please use the 'Compute' button to "
                  "generate the depreciation table."))
        return None

    _defaults = {
        'type': 'depreciate',
        'previous_id': _get_previous_id,
    }
