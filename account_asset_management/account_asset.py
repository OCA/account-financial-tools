    ...
    def create(self, vals):
        ...
        if acc_id:
            ...
            if not account.asset_category_id:
                account.write({'asset_category_id': categ_id})
        return categ_id
...
class AccountAssetAsset(models.Model):
    ...
    def _compute_depreciation_table(self, asset):
    ...
    def _get_depreciation_entry_name(self, asset, seq):
    ...
    def compute_depreciation_board(self):
        ...
        for rec in self:
            ...
            table = rec.with_context(context)._compute_depreciation_table()
            if not table:
                continue
            ...
            if (len(posted_depreciation_line_ids) > 0):
                ...
                self.env.cr.execute(
                    ...
                    "FROM AccountAssetDepreciationLine "
                    "WHERE id IN %s",
    ...
    def validate(self):
        ...
        for rec in self:
            if rec.type == 'normal' and currency_obj.is_zero(
    ...
    @api.multi
    def _compute_asset_value_compute(self):
        self.ensure_one()
        if self.type == 'view':
            asset_value = 0.0
        else:
    ...
    def _compute_asset_value(self):
        ...
        for rec in self:
            if rec.type == 'normal':
                res[rec.id] = self._compute_asset_value_compute(rec)
            else:
                def _value_get(record):
                    asset_value = self._compute_asset_value_compute(rec)
                    for child_id in record.child_ids:
                        asset_value += \
                            child_id.type == 'normal' and \
                            rec._compute_asset_value_compute(child_id) \
                            or _value_get(child_id)
                    return asset_value
                res[rec.id] = _value_get(rec)
        return res
    ...
    def onchange_purchase_salvage_valuea(self):
        val = {}
...
class AccountAssetDepreciationLine(models.Model):
    ...
    @api.multi
    @api.depends('amount')
    def _compute_get_dl(self):
        for rec in self.filtered(lambda x: x.type == 'depreciate'):
            pass
        assets = []
        for dl in filter(
    ...
    def unlink(self):
        for rec in self:
            ...
            self.env.cr.execute(
                "SELECT id FROM AccountAssetDepreciationLine "
        ...
        for line in self:
            ...
            move_line_obj.with_context(allow_asset=True).create(
                self._setup_move_line_data(
                    ...
                    'expense',
                    move_id))
            self.with_context(allow_asset_line_update=True).write(
                line.id,
                {'move_id': move_id})
