...
class AccountSpread(models.Model):
    ...
    def _compute_amounts(self):
        for spread in self:
            lines_move = spread.line_ids.filtered(lambda spread: spread.move_id)
            moves_amount = sum(spread_line.amount for spread_line in lines_move)
            lines_posted = lines_move.filtered(
                lambda spread_line: spread_line.move_id.state == "posted"
            )
    ...
    def _compute_spread_board(self):
        ...
        ).sorted(key=lambda spread_line: spread_line.date)
    ...
    def _reconcile_spread_moves(self, created_moves=False):
        ...
        mls_to_reconcile = spread_mls.filtered(
            lambda move_line: move_line.account_id == account
        )
        ...
        if mls_to_reconcile:
            ...
            do_reconcile = do_reconcile.filtered(
                lambda move_line: move_line.move_id.state == "posted"
            )
    ...
    def create_all_moves(self):
        for line in self.mapped("line_ids").filtered(
            lambda spread_line: not spread_line.move_id
        ):
            line.create_move()

    def _post_spread_moves(self, moves):
        self.ensure_one()
        moves = moves.filtered(lambda spread: spread.state != "posted")
        if not moves:
            return
