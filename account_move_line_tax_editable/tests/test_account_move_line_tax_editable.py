...
class TestAccountMoveLineTaxEditable(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.tax15 = cls.env["account.tax"].create({"name": "Test tax 15", "amount": 15})
    ...
    def test_tax_edited(self):
        """We set force_tax_line_id, after posting the move, tax_line_id should have
        been modified as the former"""
        line1 = self.move.line_ids[0]
        line1.force_tax_line_id = self.tax15.id
    ...
    def test_tax_not_edited(self):
        ...
        move came from an invoice with tax applied. Since force_tax_line_id is not set,
        tax_line_id should be computed"""
        line1 = self.move.line_ids[1]
        line1.tax_repartition_line_id = self.tax15.invoice_repartition_line_ids[1]
        self.assertEqual(line1.force_tax_line_id.id, False)
        self.move.action_post()
        self.assertEqual(line1.tax_line_id.id, self.tax15.id)
