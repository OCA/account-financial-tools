...
class CreditControlLine(models.Model):
    ...
    date_entry = fields.Date(
        string="Entry date", related="move_line_id.date", store=True, readonly=True
    )
    date_sent = fields.Date(
        string="Sent date", readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    mail_message_id = fields.Many2one("mail.mail", string="Sent Email", readonly=True)
    move_line_id = fields.Many2one(
        "account.move.line", string="Move line", required=True, readonly=True
    )
    account_id = fields.Many2one(
        "account.account", related="move_line_id.account_id", store=True, readonly=True
    )
    currency_id = fields.Many2one(
        "res.currency", related="move_line_id.currency_id", store=True, readonly=True
    )
    company_id = fields.Many2one(
        "res.company", related="move_line_id.company_id", store=True, readonly=True
    )
    ...
    level = fields.Integer(related="policy_level_id.level", store=True, readonly=True)
    ...
    @api.model
    def _prepare_from_move_line(self, move_line, level, controlling_date, open_amount):
    ...
    def create_or_update_from_mv_lines(
        self, lines, level, controlling_date, check_tolerance=True
    ):
