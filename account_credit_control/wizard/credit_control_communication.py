...
class CreditCommunication(models.TransientModel):
    ...
    current_policy_level = fields.Many2one(
        "credit.control.policy.level", "Level", required=True
    )
    currency_id = fields.Many2one("res.currency", "Currency", required=True)
    credit_control_line_ids = fields.Many2many(
        "credit.control.line", rel="comm_credit_rel", string="Credit Lines"
    )
    contact_address = fields.Many2one("res.partner", readonly=True)
