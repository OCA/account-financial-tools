...
class testCounterpart(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.currency_swiss_id = model_data_obj.get_object_reference(
    ...
    def test_reconcile_1(self):
        ...
        # We update the currency rate of the currency USD in order to force the
        # gain/loss exchanges in next steps
        ...
        invoice_id = self.account_invoice_model.create(cr, uid, {
            ...
            'invoice_line': [
                (0, 0, {
                    ...
                    'quantity': 1,
                    'price_unit': 2,
                    'account_analytic_id': self.agrolait
                })
            ]
        })
        ...
        self.assertEquals(
            invoice.state, 'paid', 'The invoice should be paid by now')
        reconcile = None
        for payment in invoice.payment_ids:
            reconcile = payment.reconcile_id
            break
        # The invoice should be reconciled (entirely, not a partial
        # reconciliation)
        ...
        for line in reconcile.line_id:
            ...
            res_account['count'] += 1
        # The journal items of the reconciliation should have their
        # debit and credit total equal
    ...
    def test_reconcile_2(self):
        ...
        for statement_line in statement.line_ids:
            self.acc_bank_stmt_line_model.process_reconciliation(
                ...
                [{
                    'counterpart_move_line_id': line_id.id,
                    'credit': 2.0, 'debit': 0.0, 'name': line_id.name
                }]
            )
