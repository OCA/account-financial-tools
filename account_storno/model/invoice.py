    ...
    def _amount_residual(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, SUPERUSER_ID, ids, context=context):
            ...
            if invoice.journal_id.posting_policy == 'storno':
                ...
                if invoice.move_id:
                    inv_curr = invoice.currency_id.id
                    for aml in invoice.move_id.line_id:
                        if aml.account_id.id == invoice.account_id.id:
                            if aml.currency_id:
                                if aml.currency_id.id == inv_curr:
                                    result[invoice.id] += \
                            ...
                            recs = aml.reconcile_partial_id.line_partial_ids
                            if recs:
                                ...
                                for line in recs:
                                    if line.invoice:
                                        if invoice.type == line.invoice.type:
                                            nb_inv_in_partial_rec += 1
                                            # store the max invoice id as for
                                            # this invoice we will make a
                                            # balance instead of a simple
                                            # division
                                            max_invoice_id = \
                                                max(max_invoice_id,
                                                    line.invoice.id)
                if nb_inv_in_partial_rec:
                    ...
                    if invoice.id == max_invoice_id:
                        ...
                        rec = (nb_inv_in_partial_rec - 1) * new_value
                        result[invoice.id] = result[invoice.id] - rec
                    else:
