...
class AccountMove(models.Model):
    ...
    @api.model
    def _move_lines_reverse_prepare(
        self, move, date=False, journal=False, line_prefix=False
    ):
        for line in move.get("line_ids", []):
            ...
            amount_currency = line[2].get("amount_currency", 0.0)
    ...
    @api.model
    def _move_reverse_prepare(self, date=False, journal=False, move_prefix=False):
        self.ensure_one()
    ...
    def move_reverse_reconcile(self):
        for move in self.filtered("reversal_id"):
            ...
            for line in lines:
                rec.setdefault(
                    (line.account_id, line.partner_id), self.env["account.move.line"]
                )
