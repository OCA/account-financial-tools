...
class account_importer_wizard(osv.osv_memory):
    """
    Account Importer
    ...
    _columns = {
        ...
        'overwrite': fields.boolean('Overwrite',
                                    help="If the account already\ exists, overwrite its name?"),
        #
        # Input file
        #
    ...
    def _find_brother_account_id(self, cr, uid, wiz, account_code, context=None):
        ...
        It will remove the last digit of the code until it finds
        an account that matches the begin of the code.
        """
        if len(account_code) > 0:
                    ...
                    if re.match(wiz.csv_code_regexp, record_code) \
                        ...
                        if account_ids:
                            if wiz.overwrite:
                                ...
                                self.pool.get('account.account').write(cr,
                                                                       uid, account_ids, {
                                                                       'name': record_name
                                                                       })
                            ...
                            #
                            parent_account_id = self._find_parent_account_id(
                                cr,
                                uid, wiz, record_code)

                            if not parent_account_id:
                                logger.warning("Couldn't find a parent\
                                    account for: %s" % record_code)

                            #
                            # Find the account's brother (will be used as template)
                            #
                            brother_account_id = self._find_brother_account_id(cr,
                                                                               uid, wiz, record_code)
                            ...
                            brother_account = self.pool.get(
                                'account.account').browse(cr,
                                                          uid, brother_account_id)
                            ...
                            account_id = self.pool.get(
                                'account.account').create(cr,
                                                          uid, {
                                                          'code': record_code,
                                                          'name': record_name,
                                                          ...
                                                          'active': 1,
                                                          'tax_ids': [(6, 0, [tax.id for tax
                                                                              in brother_account.tax_ids])],
                                                          'note': False,
                                                          })
