...
class revalidate_moves_wizard(osv.osv_memory):
    """
    Revalidate Account Moves Wizard
    ...
    """
    _name = "revalidate_moves_wizard"
    _description = "Revalidate Account Moves Wizard"

    _columns = {
        'company_id': fields.many2one('res.company', 'Company', required=True, readonly=True),
        'period_ids': fields.many2many('account.period', 'revalidate_moves_wizard_period_rel', 'wizard_id', 'period_id', "Periods"),
        'move_ids': fields.many2many('account.move', 'revalidate_moves_wizard_moves_rel', 'wizard_id', 'move_id', 'Moves'),
    ...
    def action_revalidate_moves(self, cr, uid, ids, context=None):
        ...
        for wiz in self.browse(cr, uid, ids, context):
            for move in wiz.move_ids:
                ...
                self.pool.get(
                    'account.move').validate(cr, uid, [move.id], context)
            self.write(cr, uid, [wiz.id], {'state': 'done'})
        return True

revalidate_moves_wizard()
