...
class TestPaymentIntransitReversal(SavepointCase):
    ...
    def test_journal_invoice_cancel_reversal(self):
        ...
        """
        # Test journal
        self.journal_account.write({'update_posted': True,
                                    'cancel_method': 'reversal'})
        self.assertTrue(self.journal_account.is_cancel_reversal)
        ...
        res = payment_intransit.action_intransit_cancel()
        self.assertEqual(res['res_model'], 'reverse.account.document')
        # Cancel invoice
        ctx = {'active_model': 'account.payment.intransit',
               'active_ids': [payment_intransit.id]}
        f = Form(self.env[res['res_model']].with_context(ctx))
        cancel_wizard = f.save()
        cancel_wizard.action_cancel()
        self.assertEqual(payment_intransit.state, 'cancel')
