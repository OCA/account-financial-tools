...
class AccountMoveLine(models.Model):
    ...
    def _prepare_writeoff_first_line_values(self, values):
        ...
        if 'tax_ids' in line_values:
            ...
            # _logger.info("WRITE OFF %s" % line_values)
    ...
    def _apply_taxes(self, vals, amount):
        ...
        for tax_vals in res['taxes']:
            if tax_vals['amount']:
                ...
                if 'tax_type_deal' in tax._fields and 'tax_type_deal' in self._fields and self. \
                        _get_taxes_values(tax, self.tax_type_deal):
                    continue
                tax_sign = 1
                sale = tax.type_tax_use == 'sale'
                purchase = tax.type_tax_use == 'purchase'
                if tax.type_tax_use == 'none':
                    self._cr.execute("SELECT parent_tax FROM account_tax_filiation_rel WHERE child_tax=%s",
                                     (tax.id,))
                    for parent_tax in self._cr.fetchall():
                        tax_parent = self.env['account.tax'].search([('id', '=', parent_tax)])
                        # _logger.info("TAX %s:%s" % (tax_parent, parent_tax))
                        sale = tax_parent.type_tax_use == 'sale'
                        purchase = tax_parent.type_tax_use == 'purchase'
                account_id = (amount > 0 and tax_vals['account_id'] or tax_vals['refund_account_id'])
                if (sale and tax_vals['amount'] > 0) or (purchase and tax_vals['amount'] < 0):
                    tax_sign = -1
                if not account_id:
