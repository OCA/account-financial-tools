...
class TestAccountMoveDraft(BaseCommon):
    ...
    def test_action_draft(self):
        ...
        with self.assertRaises(UserError):
            self.invoice.action_button_draft_bypass()

        self.env.user.groups_id |= self.group
        self.invoice.invoice_line_ids.invalidate_recordset()
        messages_before = self.invoice.message_ids
        self.invoice.action_button_draft_bypass()

        self.assertTrue(self.invoice.reset_to_draft_bypassed)
        messages = self.invoice.message_ids - messages_before
        self.assertIn(
