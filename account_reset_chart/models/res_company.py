        ...
        for rec in self:
            def unlink_from_company(model):
                ...
                self.cr.execute(
                    ...
                    """, obj._table, model, rec.id)
                if self.cr.rowcount:
                    logger.info(
                        "Unlinked %s properties that refer to records of type "
                        "%s that are linked to company %s",
                        self.cr.rowcount, model, rec.name)
                records = obj.search([('company_id', '=', rec.id)])
                if records:  # account_account.unlink() breaks on empty id list
                    records.unlink()

            self.env['account.journal'].search(
                [('company_id', '=', rec.id)]).write({'update_posted': True})
            ...
            self.env['account.fiscal.position.tax'].search(
                ['|', ('tax_src_id.company_id', '=', rec.id),
                 ('tax_dest_id.company_id', '=', rec.id)]
            ).unlink()
            self.env['account.fiscal.position.account'].search(
                ['|', ('account_src_id.company_id', '=', rec.id),
                 ('account_dest_id.company_id', '=', rec.id)]
            ).unlink()
