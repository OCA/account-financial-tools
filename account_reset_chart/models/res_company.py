...
class Company(models.Model):
    ...
    def reset_chart(self):
        ...
        self.env["account.journal"].search([("company_id", "=", self.id)]).write(
            {"update_posted": True}
        )
        ...
        try:
            ...
            vouchers = voucher_obj.search(
                [("company_id", "=", self.id), ("state", "in", ("proforma", "posted"))]
            )
            vouchers.cancel_voucher()
            voucher_obj.search([("company_id", "=", self.id)]).unlink()
        except KeyError:
            pass
        ...
        try:
            if self.env["payment.order"]:
                ...
                self._cr.execute(
                    "DELETE FROM payment_order WHERE company_id = %s;", (self.id,)
                )
        ...
        rec_obj.search([("line_id.company_id", "=", self.id)]).unlink()
        ...
        inv_ids = self.env["account.invoice"].search([("company_id", "=", self.id)]).ids
        if inv_ids:
