...
class AccountMoveLine(models.Model):
    ...
    def _check_invoice_related_move(self, vals=None):
        ...
        for line in self:
            # Check if move related is linked to an invoice
            invoice_linked = account_invoice_obj.search(
                [('move_id', '=', line.move_id.id)])
            if invoice_linked:
                if self._authorized_reconcile(vals):
                    return True
                err_msg = (_('Invoice name (id): %s (%s)') %
                            (invoice_linked.move_name, invoice_linked.id))
    ...
    @api.multi
    def _check_payment_related_move(self, vals=None):
        for line in self:
            if line.payment_id:
                if self._authorized_reconcile(vals):
                    return True
                err_msg = (_('Payment Order name (id): %s (%s)') %
                            (line.payment_id.name, line.payment_id.id))
                raise exceptions.Warning(
                    _('You cannot do this on an entry generated by a payment'
                      ' order. '
                      'You must change the related payment order'
                      ' directly.\n%s.') % err_msg)
        return True
    ...
    def unlink(self, cr, uid, ids, context=None, check=True):
        ...
        - Is the move related to a payment order
