...
##############################################################################
{
    ...
    * Add a constraints on reconcile object to forbid the reconciliation between different partner
      
    * Add a constraint on account move: you cannot pickup a date that is not in the fiscal year of the concerned period (Not in 7.0)
    ...
    * Add contraint for Payment Order : If a invoice is imported in a payment order, forbid to reset invoice to cancel or draft
    
    * Forbid to remove the reconcile on opening entries, we introduce a new boolean field to identify the reconciliation made by the closing process from others.
