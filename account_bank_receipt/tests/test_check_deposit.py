...
class TestPayment(AccountingTestCase):
    ...
    def setUp(self):
        ...
        self.check_deposit_model = self.env['account.bank.receipt']
        self.receipt_line_model = self.env['account.bank.receipt.line']
    ...
    def create_check_deposit(self, move_lines):
        ...
        for move_line in move_lines:
            self.receipt_line_model.create({
                'account_move_line_ids': move_line.id,
                'type': 'cash',
                'allocation': 300,
                'order_id': check_deposit.id
            })
