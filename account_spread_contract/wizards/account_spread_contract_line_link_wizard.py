...
class AccountSpreadContractLineLinkWizard(models.TransientModel):
    ...
    contract_id = fields.Many2one(
        related="contract_line_id.analytic_account_id", readonly=True
    )
    contract_type = fields.Selection(
        related="contract_line_id.analytic_account_id.contract_type", readonly=True
    )
    spread_template_id = fields.Many2one(
        "account.spread.template", string="Spread Template", required=True
    )
