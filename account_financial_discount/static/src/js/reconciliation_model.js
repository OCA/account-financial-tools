odoo.define("account_financial_discount.ReconciliationModel", function(require) {
    ...
    ReconciliationModel.StatementModel.include({
        _formatFinancialDiscountTaxQuickCreate: function(line) {
            ...
            var tax_account =
                line.reconciliation_proposition[0].discount_tax_line_account_id;
        ...
        _formatFinancialDiscountQuickCreate: function(line, values) {
            ...
            var today = new moment().utc().format();
            var amount_discount =
                -line.reconciliation_proposition[0].amount_discount +
                line.reconciliation_proposition[0].amount_discount_tax;
            var fin_disc_account =
                amount_discount < 0
                    ? values.financial_discount_expense_account_id
                    : values.financial_discount_revenue_account_id;
        ...
        quickCreateProposition: function(handle, reconcileModelId) {
            ...
            if (
                ...
                line.reconciliation_proposition[0].financial_discount_available &&
                !line.createForm
            ) {
                ...
                var focus = this._formatFinancialDiscountQuickCreate(
                    line,
                    _.pick(reconcileModel, fields)
                );
                ...
                if (line.reconciliation_proposition[0].amount_discount_tax) {
                    defs.push(
                        self._computeLine(line).then(function() {
                            var tax_focus = self._formatFinancialDiscountTaxQuickCreate(
                                line
                            );
