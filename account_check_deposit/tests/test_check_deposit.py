...
class TestPayment(AccountingTestCase):
    def setUp(self):
        ...
        self.main_company.check_deposit_account_id = self.account_account_model.search(
            [("code", "=", "511201")], limit=1
        )
        if not self.main_company.check_deposit_account_id:
            self.main_company.check_deposit_account_id = self.account_account_model.create(
                {
                    "code": "511201",
