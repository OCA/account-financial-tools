...
class AccountCheckDeposit(models.Model):
    ...
    line_ids = fields.One2many(
        "account.move.line", related="move_id.line_ids", string="Lines", readonly=True
    )
    ...
    check_count = fields.Integer(
        compute="_compute_check_deposit", readonly=True, string="Number of Checks"
    )
    is_reconcile = fields.Boolean(
        compute="_compute_check_deposit", readonly=True, string="Reconcile"
    )
    ...
    def create(self, vals):
        if vals.get("name", "/") == "/":
            vals["name"] = self.env["ir.sequence"].next_by_code("account.check.deposit")
    ...
    def _prepare_counterpart_move_lines_vals(
        ...
        if company.check_deposit_offsetting_account == "bank_account":
            ...
            if not company.check_deposit_transfer_account_id:
                raise UserError(
                    _("Missing 'Account for Check Deposits' on the " "company '%s'.")
                    % company.name
                )
