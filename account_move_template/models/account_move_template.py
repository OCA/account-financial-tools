...
class AccountMoveTemplate(models.Model):
    ...
    def action_run_template(self):
        self.ensure_one()
        action = self.env.ref(
            "account_move_template.action_wizard_select_template"
        ).read()[0]
...
class AccountMoveTemplateLine(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", related="template_id.company_id", readonly=True
    )
    journal_id = fields.Many2one(
        "account.journal", related="template_id.journal_id", store=True, readonly=True
    )
    account_id = fields.Many2one("account.account", required=True, ondelete="cascade")
    partner_id = fields.Many2one("res.partner", string="Partner")
    move_line_type = fields.Selection(
        [("cr", "Credit"), ("dr", "Debit")], required=True
    )
    analytic_account_id = fields.Many2one(
        "account.analytic.account", ondelete="cascade"
    )
