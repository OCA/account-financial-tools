...
class HrHnbGetter(CurrencyGetterInterface):
    ...
    def get_updated_currency(self, currency_array, main_currency,
        ...
        rate_date = datetime.now().strftime(DEFAULT_SERVER_DATE_FORMAT)
        date_str = datetime.strftime(datetime.strptime(
            rate_date, DEFAULT_SERVER_DATE_FORMAT), '%d%m%y')
        ...
        for line in rawfile.splitlines():
            ...
            if len(line) == 4:
                ...
                # all rates, but only use rate_avg
                curr_rates[curr] = {
                    'rate_buy': round(
                        num_of_units / float(line[1].replace(',', '.')), 6),
                    'rate_avg': round(
                        num_of_units / float(line[2].replace(',', '.')), 6),
                    'rate_sell': round(
                        num_of_units / float(line[3].replace(',', '.')), 6),
                }

        for curr in currency_array:
            self.validate_cur(curr)
            if curr in curr_rates.keys():
                self.updated_currency[curr] = curr_rates[curr]['rate_avg']
                _logger.debug('Rate retrieved: %s = %s %s' % (
                    main_currency, curr_rates[curr]['rate_avg'], curr))
