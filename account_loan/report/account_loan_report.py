...
class AccountLoanReport(models.Model):
    ...
    name = fields.Char(
        string='Loan', required=False, readonly=True,
        )
    description = fields.Char(
        string='Description', readonly=True,
        )
    partner_id = fields.Many2one(
        'res.partner', string='Lender', readonly=True,
        )
    company_id = fields.Many2one(
        'res.company', string='Company', readonly=True,
        )
    periods = fields.Integer(
        string='# of Loan Periods', readonly=True,
        )
    method_period = fields.Integer(
        string='Time Between Loan Periods', readonly=True,
        )
    rate = fields.Float(
        string='Loan Rate', readonly=True,
        )
    pending_principal_amount = fields.Float(
        string='Amount of Pending Principal', readonly=True,
        )
    payment_amount = fields.Float(
        string='Amount of Payment', readonly=True,
        )
    interests_amount = fields.Float(
        string='Amount of Interests', readonly=True,
        )
    ...
    loan_type = fields.Selection([
        ...
        ('interest', 'Only interest')], readonly=True,
        )
    start_date = fields.Date(
        string='Loan Date', readonly=True,
        )
    ...
    loan_amount = fields.Float(
        string='Loan Amount', readonly=True,
        )
    short_term_loan_account_id = fields.Many2one(
        'account.account', string='Short term account', readonly=True,
        )
    long_term_loan_account_id = fields.Many2one(
        'account.account', string='Long term account', readonly=True,
        )
    ...
    def _select(self):
        select_str = """
            SELECT
                ...
                (al.loan_amount - SUM(ll.payment_amount)
                + SUM(ll.interests_amount)) as pending_principal_amount,
                al.state as state,
