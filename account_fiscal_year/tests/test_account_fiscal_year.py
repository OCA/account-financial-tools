...
class TestAccountFiscalYear(TransactionCase):
    ...
    def test_compute_fiscalyear(self):
        ...
        self.assertEqual(result["date_from"], datetime.date(self.today_date.year, 1, 1))
        self.assertEqual(result["date_to"], datetime.date(self.today_date.year, 12, 31))
        ...
        self.assertEqual(
            result["date_from"], fields.Date.from_string(demo_date_range.date_start)
        )
        self.assertEqual(
            result["date_to"], fields.Date.from_string(demo_date_range.date_end)
        )
